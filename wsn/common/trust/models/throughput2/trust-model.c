...
{
    const throughput_dist_t* in = &capability->tm.throughput_in;
    const throughput_dist_t* out = &capability->tm.throughput_out;
    ...
    {
        if (out->current < info->out_threshold)
        {
            return 0;
        }
        else
    ...
    else
    {
        ...
        if (in->current >= info->in_threshold && out->current >= info->out_threshold)
        {
            return 1;
        }
        else if (in->current < info->in_threshold && out->current > ((info->out_threshold/info->in_threshold)*in->current) )
        {
            return (in->current)/info->in_threshold;
        }
        else
        {
            return (out->current)/info->out_threshold;
        }
