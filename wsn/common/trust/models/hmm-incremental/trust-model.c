...
float calculate_trust_value(edge_resource_t* edge, edge_capability_t* capability)
{
    ...
    return hmm_one_observation_probability(&edge->tm.hmm, HMM_OBS_TASK_RESULT_QUALITY_CORRECT);
}
/*-------------------------------------------------------------------------------------------------------------------*/
void tm_update_task_submission(edge_resource_t* edge, edge_capability_t* cap, const tm_task_submission_info_t* info)
{
    ...
    if (!good)
    {
        hmm_update(&edge->tm.hmm, HMM_OBS_TASK_SUBMISSION_ACK_TIMEDOUT, edge->tm.first);
        edge->tm.first = false;
    }
...
void tm_update_task_result(edge_resource_t* edge, edge_capability_t* cap, const tm_task_result_info_t* info)
{
    ...
    {
        hmm_update(&edge->tm.hmm, HMM_OBS_TASK_RESPONSE_TIMEDOUT, edge->tm.first);
        edge->tm.first = false;
    }
...
void tm_update_result_quality(edge_resource_t* edge, edge_capability_t* cap, const tm_result_quality_info_t* info)
{
    ...
    if (info->good)
    {
        hmm_update(&edge->tm.hmm, HMM_OBS_TASK_RESULT_QUALITY_CORRECT, edge->tm.first);
        edge->tm.first = false;
    }
    else
    {
        hmm_update(&edge->tm.hmm, HMM_OBS_TASK_RESULT_QUALITY_INCORRECT, edge->tm.first);
        edge->tm.first = false;
    }
...
int serialise_trust_edge_resource(nanocbor_encoder_t* enc, const edge_resource_tm_t* edge)
{
    ...
    NANOCBOR_CHECK(hmm_serialise(enc, &edge->hmm));
    NANOCBOR_CHECK(nanocbor_fmt_bool(enc, edge->first));
...
{
    NANOCBOR_CHECK(hmm_deserialise(dec, &edge->hmm));

    nanocbor_value_t arr;
    NANOCBOR_CHECK(hmm_deserialise(&arr, &edge->hmm));
    NANOCBOR_CHECK(nanocbor_get_bool(&arr, &edge->first));
