...
float calculate_trust_value(edge_resource_t* edge, edge_capability_t* capability)
{
    ...
    return hmm_observation_probability(&edge->tm.hmm, HMM_OBS_TASK_RESULT_QUALITY_CORRECT, &edge->tm.hist);
}
/*-------------------------------------------------------------------------------------------------------------------*/
void tm_update_task_submission(edge_resource_t* edge, edge_capability_t* cap, const tm_task_submission_info_t* info)
{
    ...
    if (!good)
    {
        interaction_history_push(&edge->tm.hist, HMM_OBS_TASK_SUBMISSION_ACK_TIMEDOUT);
    }
...
void tm_update_task_result(edge_resource_t* edge, edge_capability_t* cap, const tm_task_result_info_t* info)
{
    ...
    {
        interaction_history_push(&edge->tm.hist, HMM_OBS_TASK_RESPONSE_TIMEDOUT);
    }
...
void tm_update_result_quality(edge_resource_t* edge, edge_capability_t* cap, const tm_result_quality_info_t* info)
{
    ...
    if (info->good)
    {
        interaction_history_push(&edge->tm.hist, HMM_OBS_TASK_RESULT_QUALITY_CORRECT);
    }
    else
    {
        interaction_history_push(&edge->tm.hist, HMM_OBS_TASK_RESULT_QUALITY_INCORRECT);
    }
